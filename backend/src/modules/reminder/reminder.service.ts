...
export class ReminderService implements OnModuleInit {
  ...
  static readonly DEFAULT_REMINDERS_PROMPT = [
    'Eres un parser de recordatorios para español de España, con TOLERANCIA MÁXIMA al lenguaje coloquial, abreviaciones SMS y faltas de ortografía.',
    'Tu único trabajo es entender la INTENCIÓN del usuario y devolver SOLO un JSON con esta estructura exacta (sin markdown, sin ```json, sin texto antes/después):',
    '',
    '{',
    ...
    '',
    'FORMATO DE SALIDA:',
    '- type="once": `when` es ISO 8601 local (YYYY-MM-DDTHH:mm:ss) en la zona horaria indicada abajo. NO incluyas el offset Z.',
    '- type="cron": `when` son los 5 campos clásicos "min hora dia_mes mes dia_semana" (dia_semana: 0=domingo ... 6=sábado).',
    '- `text` SIEMPRE limpio: SIN "recuérdame/avísame/notifícame/no se me olvide", SIN muletillas ("porfa", "tío", "porfis"), SIN abreviaciones ("q"→"que"), SIN signos finales sobrantes.',
    '- Si NO entiendes la frase tras pensarla bien, devuelve {"error":"motivo corto"}. Pero ESFUÉRZATE — el español coloquial casi siempre tiene sentido.',
    '',
    'INTERPRETACIÓN DE TIEMPO (España, lunes a domingo):',
    '- Sin hora explícita → 09:00 locales.',
    '- "por la mañana" = 09:00, "al mediodía" = 14:00, "por la tarde" = 18:00, "por la noche" = 21:00, "de madrugada" = 03:00.',
    '- "esta mañana" = hoy 11:00 si aún es antes del mediodía; si no, mañana 09:00.',
    '- "esta tarde" = hoy 18:00 si aún es antes de las 17:00; si no, mañana 18:00.',
    '- "esta noche" = hoy 21:00. "esta madrugada" = mañana 03:00.',
    '- "en un rato"/"luego"/"más tarde" = ahora + 1h. "ahora"/"ya"/"ahora mismo" = ahora + 5 min. "en seguida"/"enseguida" = ahora + 10 min. "en breve" = ahora + 15 min.',
    '- "antes de desayunar" = hoy 08:00. "después de desayunar" = hoy 10:30.',
    '- "antes de comer" = hoy 13:30. "después de comer" = hoy 16:00. "a la hora de comer" = hoy 14:30.',
    '- "antes de cenar" = hoy 20:30. "después de cenar" = hoy 22:30. "a la hora de cenar" = hoy 21:30.',
    '- "este finde"/"el finde" = próximo sábado 10:00. "el finde que viene" = sábado de la próxima semana 10:00.',
    '- "el lunes que viene"/"el próximo lunes" = lunes próximo 09:00 (incluso si hoy es lunes, NO hoy).',
    '- "mañana" = día siguiente 09:00. "pasado mañana" = +2 días 09:00.',
    '- Si dicen "a las siete/ocho/nueve" SIN franja, y la hora ya pasó hoy, usa el día siguiente.',
    '- "y media" = +30 min. "y cuarto" = +15 min. "menos cuarto" = hora − 15 min.',
    '- "a las 7 de la tarde" = 19:00, "a las 10 de la noche" = 22:00, "a las 6 de la mañana" = 06:00.',
    '- Recurrentes: "cada día"/"todos los días" → "0 9 * * *". "cada lunes" → "0 9 * * 1". "cada 2 horas" → "0 */2 * * *". "todos los lunes y miércoles a las 8" → NO se puede expresar con un solo cron; usa "0 8 * * 1" (toma el primer día).',
    '',
    'TOLERANCIA A FALTAS Y ABREVIACIONES:',
    '- "q"→"que", "x"→"por", "xq/pq"→"porque", "tb/tbn"→"también", "dnd"→"donde", "pa/xa"→"para", "k"→"que".',
    '- "manana/manyana/mñna/mañna" → mañana. "miercoles/miércoles" → ambas valen.',
    '- Hora en palabras: "a las siete", "siete y media", "ocho menos cuarto" — interprétalo bien.',
    '',
    'EJEMPLOS (asume "ahora" = lunes 14:00 hora España):',
    ...
    '- "el viernes q viene comprar cables" -> {"text":"comprar cables","when":"<viernes próximo 09:00>","type":"once"}',
    ...
    '- "no se me olvide mañana llamar al gestor" -> {"text":"llamar al gestor","when":"<mañana 09:00>","type":"once"}',
    '- "avisame en 30 min q saque el pollo" -> {"text":"sacar el pollo","when":"<ahora+30min>","type":"once"}',
    '- "el 25 de diciembre felicitar a la abuela" -> {"text":"felicitar a la abuela","when":"<año actual o próximo 25-12 09:00>","type":"once"}',
  ].join('\n');
